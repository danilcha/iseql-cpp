...
public:
        ...
        Bp(r2.size())
    {
        ...
        if (op1 == JoinCondition::GreaterThan || op1 == JoinCondition::GreaterThanEqual)
        {
            sortDesc(L1);
            sortDesc(L1p);
        }
        ...
        if (op2 == JoinCondition::GreaterThan || op2 == JoinCondition::GreaterThanEqual)
        {
            sortAsc(L2);
            sortAsc(L2p);
        }
        ...
        O1 = computeOffsets(L1, L1p);
        O2 = computeOffsets(L2, L2p);

        if ((op1 == JoinCondition::LessThanEqual || op1 == JoinCondition::GreaterThanEqual) &&
            (op2 == JoinCondition::LessThanEqual || op2 == JoinCondition::GreaterThanEqual))
        {
            eqOff = 0;
        }
        else
    ...
    void join(Consumer&& consumer)
    {
        for (size_t i = 0; i < r1.size(); i++)
        {
            ...
            for (size_t j = 0; j <= std::min(off2, r2.size() - 1); j++)
            {
                auto p = Pp[j];
                Bp.setBit(p);
            }
            ...
            for (auto k = off1 + eqOff; k < r2.size(); k++)
            {
                if (Bp.isSet(k))
...
private:
    ...
    static void sortDesc(Elements& elements)
    {
        throw new std::runtime_error("Sorry, offset computation does not support this order");
    ...
    static Indices computeOffsets(const Elements& from, const Elements& to)
    {
        auto f = from.begin();
        auto t = to.begin();
